...
def main(config_name: str, max_frames: int | None = None, save_dir: str | None = None):
    ...
    for batch in tqdm.tqdm(data_loader, total=num_frames, desc="Computing stats"):
        # print(batch.keys())
        state_values = np.asarray(batch["state"][0])
        action_values = np.asarray(batch["actions"][0])
    
        # 更新统计量
        stats["state"].update(state_values.reshape(-1, state_values.shape[-1]))
        stats["actions"].update(action_values.reshape(-1, action_values.shape[-1]))

        # 对比特定维度 [14,15,16,17,18,19,20,21]
        selected_dims = list(range(14, 21))
        

        state_selected = state_values[..., selected_dims]
        action_selected = action_values[..., selected_dims]

        # 打印维度差值（可以改成打印 mean diff、绝对差、最大差等）
        diff = state_selected - action_selected
        # print(f"\n[Batch Diff 14~21]  diff per dim: {diff}\n")
    ...
    if save_dir is None:
        ...
        normalize.save(save_dir, norm_stats)
